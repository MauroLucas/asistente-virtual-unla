...
EXPECTED = [
    ...
    "cuatrimestre", "Carga Horaria Semanal", "Carga Horaria Total"
]
...
def ensure_table(conn):
    ...
    CREATE TABLE IF NOT EXISTS data.materias_sistemas_plan_2014 (
        "Carrera"                 TEXT,
        "Plan"                    TEXT,
        "Materia"                 TEXT,
        "Correlativas"            JSON,
...
def load_excel_to_table(conn, xlsx_path):
    ...
    for c in ["Carrera","Plan","Materia","cuatrimestre","Carga Horaria Semanal","Carga Horaria Total"]:
        def _to_text_or_none(x):
            if isinstance(x, str):
                xs = x.strip()
                return None if xs == "" else xs
            return None if pd.isna(x) else str(x)
        df[c] = df[c].apply(_to_text_or_none)
    ...
    for _, r in df.iterrows():
        rows.append((
            ...
            r["cuatrimestre"], r["Carga Horaria Semanal"], r["Carga Horaria Total"]
        ))

    with conn.cursor() as cur:
        # vacía la tabla
        cur.execute('TRUNCATE TABLE data.materias_sistemas_plan_2014;')
        execute_values(
            cur,
            """
            INSERT INTO data.materias_sistemas_plan_2014
...
def run(conn, data_dir):
    ...
    try:
        ensure_table(conn)
        xlsx = os.path.join(data_dir or "/app/data", "materias_sistemas_plan_2014.xlsx")
        ...
        print("✅ Carga de data.materias_sistemas_plan_2014 completada.")
